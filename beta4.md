...
- Global DXVK Variant selector removed from Settings page — per-game DXVK combo now shows Off/Development/Stable/Lilium HDR directly (no "Global" indirection).
...
- Fixed DXVK Variant Changed dialog appearing every time Settings page was opened (service variant wasn't synced from saved settings on startup).
...
- Added `crashpad_handler.exe` to excluded profile exe names — prevents false matches against "Adobe Substance 3D Stager" profile for UE/Chromium games (PRAGMATA, SMT5V, etc.).
