...
- Includes depth buffer preprocessor definitions (`RESHADE_DEPTH_INPUT_IS_REVERSED`, `RESHADE_DEPTH_INPUT_IS_LOGARITHMIC`, `RESHADE_DEPTH_LINEARIZATION_FAR_PLANE`) tuned for Vulkan rendering.
...
- Shader sync (`SyncShadersToAllLocations`) uses the footprint to determine whether to deploy `reshade-shaders/` to Vulkan game folders.
- The footprint is automatically removed when Display Commander is installed (DC manages shaders globally) and restored when DC is uninstalled from a Vulkan game.

**Per-game Vulkan ReShade uninstall**
- A ✕ uninstall button is now shown for Vulkan games that have `reshade.ini` deployed, allowing you to remove Vulkan ReShade artifacts (reshade.ini, footprint, reshade-shaders) from a specific game folder without affecting the global Vulkan layer.
- Uninstall restores `reshade-shaders-original` if present.

**Vulkan ReShade status detection**
- Vulkan games now show the ReShade version number with "(Vulkan)" underneath in the detail panel when `reshade.ini` is present, matching the green installed styling of DLL-based ReShade games.
...
**Select shader mode**
- A new "Select" shader deploy mode has been added alongside Off, Minimum, All, and User. When Select is chosen, a picker window appears listing all 7 available shader packs with checkboxes, letting you choose exactly which packs to deploy.
- The shader button cycles through Off → Minimum → All → User → Select → Off. Clicking the button while already in Select mode re-opens the picker instead of cycling away.
- The selection is saved globally and restored across app restarts.
- Per-game shader mode overrides in both Detail View and Grid View now include "Select" as an option, allowing different games to use different subsets of shader packs.

### Bug Fixes

**Drag-and-drop not working**
- Fixed drag-and-drop of game executables and archives not functioning in the unpackaged WinUI 3 app. OLE-based XAML drag/drop events (`AllowDrop`/`DragOver`/`Drop`) are unreliable in unpackaged WinUI 3, so drag-and-drop now uses Win32 shell `WM_DROPFILES` handling via the existing WndProc subclass.
- Also added `ChangeWindowMessageFilterEx` for `WM_DROPFILES` and `WM_COPYGLOBALDATA` so drag-and-drop works when RDXC is running as administrator (UIPI bypass).

**Shaders not deployed to game folders after Display Commander removal**
- Games with ReShade installed but no Display Commander were left with an empty `reshade-shaders\` folder after DC was uninstalled, because DC Mode cleanup removed game-local shaders and nothing repopulated them once DC was gone. Refresh and Deploy Shaders now correctly detect the `rsInstalled && !dcInstalled` scenario and deploy shaders to the game folder using the effective shader mode (global or per-game override). User-owned shader folders are preserved by renaming to `reshade-shaders-original` before deployment and restored when the shader mode is set to Off.
